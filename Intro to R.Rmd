---
title: "Foundations of R Programming"
...
```{r}
# if you are using RStudio you can use alt + "-" to automatically create "<-" assignment operator 
a <- 100
a
```
...
Another way to assign variables is using the assign function. The laborious way of assigning variables.
...
```{r}
y <- factor(x = "numbers", levels = c("data", "numbers", "Thakaa"))
class(y)
...
luckily we have `lubridate` package that makes it easy to deal with dates but you will have to explore it yourself. 
...
There are many ways to get help to know more about any function. for now we will learn only about `?` and `??` and later I will show you how to get more help. 
...
greetings <- function(name, lastname) {
  
  return(paste("Hello ", name, lastname))
}

greetings(name = "Hussain")
# or we can provide arguments positional 
greetings(lastname = "Hussain", name = "Alsalman")
...
spell_name <- function(name, ...) {
  ...
  if (length(argument) > 0 &
      !is.null(argument$check) & is.logical(argument$check)) {
    check <- argument$check
    if (check == TRUE) {
      spelling <- substr(name, nchar(name) - 2, nchar(name))
      return(paste0(
        ". Just to make sure, last ",
        min(nchar(name), 3) ,
        " letters of  your name ",
        ifelse(nchar(spelling) > 1, "are ", "is "),
        toupper(spelling)
      ))
    }
  }
...
{
 print("there are not the same length")
} else if (y == NULL) {
  print("y is NULL")
} else {
  print("I don't know")
...
```{r}
read.csv(file = "./assets/Data/Read_Data.csv", stringsAsFactors = FALSE, colClasses = c("character", "integer", "numeric", "factor")) 
...
```{r}
library("readxl")
file_path <- "./assets/Data/Read_Data.xlsx"
...
```{r}
save(list=ls(),file = "./assets/Data/all_data.RData")
rm(list = ls())
```

```{r}

load("./assets/Data/all_data.RData")
...
ggplot2 package is a powerful visualizing package. It was built based on the Grammar of Graphics book by Leland Wilkinson. The general idea behind this package is that you build your visualization layer by layer. 
